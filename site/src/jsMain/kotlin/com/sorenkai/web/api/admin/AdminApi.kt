package com.sorenkai.web.api.admin
...
class AdminApi(private val apiClient: ApiClient) {
    ...
    suspend fun setWritingStatus(slug: String, status: WritingsStatus): ApiResponse<AdminOk> {
        val body = js("({})")
        body.status = status.name
        return apiClient.patch("/v2/admin/writings/$slug/status", body = body) { json.decodeFromString(it) }
    }

    suspend fun setWritingFeatured(slug: String, featured: Boolean): ApiResponse<AdminOk> {
        val body = js("({})")
        body.featured = featured
        return apiClient.patch("/v2/admin/writings/$slug/featured", body = body) { json.decodeFromString(it) }
    }

    suspend fun deleteWriting(slug: String): ApiResponse<AdminOk> =
        apiClient.delete("/v2/admin/writings/$slug") { json.decodeFromString(it) }
}
